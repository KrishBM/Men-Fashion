...
public class AddShopDialog extends AppCompatDialogFragment {
    ...
    public Dialog onCreateDialog(Bundle savedInstanceState){
        ...
        View view=inflater.inflate(R.layout.add_shop,null);
...
//                        if(uploadSuccesFlag.equals("s") && !(uploadImageURL.isEmpty())){
    ...
    private void uploadImage()
    {
        if (filePath != null) {
            ...
            String path="shopLogo/" + UUID.randomUUID().toString();
...
//        Log.d("imgggggggg",imageURL);
        ...
        String shopID = shopDatabaseReference.push().getKey();
        assert shopID != null;
        shopDatabaseReference.child(shopID).setValue(shop).addOnSuccessListener(unused -> {
            // after adding this data we are showing toast message.

            //shopID stored in tailor table
            tailorDatabaseReference = firebaseDatabase.getReference().child("users");
            tailorDatabaseReference.child(FirebaseAuth.getInstance().getCurrentUser().getUid()).child("ShopID").setValue(shopID);
